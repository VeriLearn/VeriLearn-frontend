      ...
      <div className="mx-auto max-w-6xl px-6 py-10">
        ...
        <div className="flex gap-8">
          ...
          <aside className="hidden lg:block w-56 shrink-0">
            <div className="rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-zinc-900 p-4">
              <p className="mb-3 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-zinc-400">Filter</p>

              <fieldset className="mb-5">
                <legend className="mb-2 text-sm font-semibold text-gray-700 dark:text-zinc-300">Level</legend>
                <ul className="space-y-1.5">
                  {levels.map((l) => (
                    <li key={l}>
                      <label className="flex items-center gap-2 text-sm text-gray-600 dark:text-zinc-400 hover:text-indigo-600 dark:hover:text-indigo-400 cursor-pointer">
                        <input
                          type="checkbox"
              ...
              <fieldset>
                <legend className="mb-2 text-sm font-semibold text-gray-700 dark:text-zinc-300">Topic</legend>
                <ul className="space-y-1.5">
                  {topics.map((t) => (
                    <li key={t}>
                      <label className="flex items-center gap-2 text-sm text-gray-600 dark:text-zinc-400 hover:text-indigo-600 dark:hover:text-indigo-400 cursor-pointer">
          ...
          <div className="flex-1">
            ...
            <div className="mb-5 flex items-center gap-3">
              <div className="flex flex-1 items-center gap-2 rounded-lg border border-gray-300 dark:border-white/10 bg-white dark:bg-zinc-900 px-3 py-2 text-sm">
                ...
                <input
                  ...
                  className="flex-1 bg-transparent text-gray-700 dark:text-zinc-300 placeholder-gray-400 dark:placeholder-zinc-500 focus:outline-none"
                />
              </div>
              <span className="shrink-0 text-sm text-gray-500 dark:text-zinc-400">{filtered.length} results</span>
            </div>

            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
              {filtered.map((course) => (
                <Link key={course.id} href={`/courses/${course.id}`}
                  className="group flex flex-col rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-zinc-900 p-5 shadow-sm hover:shadow-md hover:border-indigo-200 dark:hover:border-indigo-500/30 transition-all">
                  <div className="mb-3 flex items-center gap-3">
                    ...
                    <span className="font-semibold text-gray-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors leading-tight text-sm">
                      {course.title}
                    </span>
                  </div>
                  <div className="mb-3 flex flex-wrap gap-1.5">
                    <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${tagColors[course.tag]}`}>{course.tag}</span>
                    <span className="rounded-full bg-gray-100 dark:bg-white/10 px-2.5 py-0.5 text-xs font-medium text-gray-500 dark:text-zinc-400">Developer course</span>
                  </div>
                  <p className="flex-1 text-xs leading-relaxed text-gray-500 dark:text-zinc-400 line-clamp-3">{course.description}</p>
                  <div className="mt-4 flex items-center gap-2 border-t border-gray-100 dark:border-white/10 pt-3">
                    <span className="text-xs text-gray-400 dark:text-zinc-500">{course.lessons.length} lessons · {course.duration}</span>
                  </div>
                </Link>
              ))}
